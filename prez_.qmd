...
4. **Straightforward to Extend to Moving Interfaces**  
...
&= \underbrace{\sum_{f \in \text{reg}} k \mathcal{A_f} \nabla_f(\tilde{T})}_{\substack{\text{space-time flux}\\\text{through regular faces}}} 
+ \underbrace{F_{\Gamma}}_{\substack{\text{space-time flux across}\\\text{moving interface}}} 
+ \underbrace{S_i V_i^{\text{avg}}}_{\text{source term}} 
\end{aligned}
$$
...
+ \underbrace{ q_{i+1/2}^{\omega}}_{\substack{\text{space-time flux} \\ \text{through faces}}} - \underbrace{ q_{i-1/2}^{\gamma}}_{\substack{\text{flux across} \\ \text{moving interface } \Gamma(t)}} = 0
\end{aligned}
$$
